...
def send_email_notification(task_list, email_config):
    ...
    required_fields = ["sender_email", "recipient_email", "sender_password"] # Use constants instead when defination is done
    missing_fields = [field for field in required_fields if not email_config.get(field)]
    if missing_fields:
        raise GoogleTaskNotificationError(f"Missing required email configuration fields: {', '.join(missing_fields)}")
    ...
    try:
        ...
        _LOGGER.info("Email notifacation sent successfully to %s with %d task(s)." ,
                     email_config["recipient_email"],
                     task_count,
        )
